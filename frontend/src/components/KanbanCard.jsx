...
export default function KanbanCard({ task, column }) {
  ...
  return (
    <motion.div
      ...
      className={`p-4 rounded-lg border cursor-grab active:cursor-grabbing shadow-sm ${
        column === 'completed' 
      ...
      {task.suggested_talking_points && (
        <div className="mt-3 pt-3 border-t dark:border-gray-700">
          <div className="flex items-start text-sm">
            ...
            <p className="text-gray-700 dark:text-gray-300">{task.suggested_talking_points}</p>
          </div>
        </div>
      )}
